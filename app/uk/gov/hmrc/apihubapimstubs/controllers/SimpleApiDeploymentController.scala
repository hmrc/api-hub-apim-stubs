...
import uk.gov.hmrc.apihubapimstubs.models.simpleapideployment._
...
class SimpleApiDeploymentController @Inject()(cc: ControllerComponents) extends BackendController(cc) with Logging {
  ...
  def validateOas(): Action[String] = Action(parse.tolerantText) {
    request =>
  ...
  def deployNewApi(): Action[MultipartFormData[Files.TemporaryFile]] = Action(parse.multipartFormData) {
    request =>
  ...
  def deployExistingApiWithNewConfiguration(serviceId: String): Action[MultipartFormData[Files.TemporaryFile]] = Action(parse.multipartFormData) {
    request =>
      (request.body.dataParts.get("metadata"), request.body.dataParts.get("openapi")) match {
        case (Some(Seq(metadata)), Some(Seq(openapi))) =>
